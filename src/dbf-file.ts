...
import {FieldDescriptor, validateFieldDescriptor} from './field-descriptor.ts';
import {isValidFileVersion} from './file-version.ts';
import {CreateOptions, Encoding, normaliseCreateOptions, normaliseOpenOptions, OpenOptions} from './options.ts';
import {close, open, read, stat, write} from './utils.ts';
import {createDate, format8CharDate, formatVfpDateTime, parseVfpDateTime, parse8CharDate} from './utils.ts';
import { BufferStream } from "./buffer-stream.ts";
...
async function openDBF(path: string, opts?: OpenOptions): Promise<DBFFile> {
    ...
    let fd: Deno.FsFile | undefined = undefined;
    try {
        // Open the file and create a buffer to read through.
        fd = await open(path, 'r');
        let buffer = BufferStream.alloc(32);
        ...
        if (fileVersion === 0x83 || fileVersion === 0x8b) {
            for (const ext of ['.dbt', '.DBT']) {
                ...
                const foundMemoFile = await stat(memoPath).catch(() => 'missing') !== 'missing';
        ...
        while (headerLength > 32 + fields.length * 32) {
            ...
            const field: FieldDescriptor = {
                ...
                type: String.fromCharCode(buffer.at(0x0B)) as FieldDescriptor['type'],
        ...
        assert(buffer.at(0) === 0x0d, 'Invalid DBF: Expected header terminator');
...
async function createDBF(path: string, fields: FieldDescriptor[], opts?: CreateOptions): Promise<DBFFile> {
    ...
    try {
        ...
        const buffer = BufferStream.alloc(32);
...
async function readRecordsFromDBF(dbf: DBFFile, maxCount: number) {
    let fd: Deno.FsFile | undefined = undefined;
    let memoFd: Deno.FsFile | undefined = undefined;
    try {
        // Open the file and prepare to create a buffer to read through.
        fd = await open(dbf.path, 'r');
        const recordCountPerBuffer = 1000;
        const recordLength = dbf._recordLength;
        const buffer = BufferStream.alloc(recordLength * recordCountPerBuffer);
        ...
        let memoBlockSize = 0;
        let memoFileSize = 0;
        let memoBuf: BufferStream | undefined;
        if (dbf._memoPath) {
            ...
            memoBuf = BufferStream.alloc(memoBlockSize);
        ...
        const substrAt = (start: number, len: number, enc: string) => iconv.decode(buffer.slice(start, start + len), enc);
        const int32At = (start: number, len: number) => buffer.slice(start, start + len).readInt32LE(0);
        ...
        while (true) {
            ...
            const maxRecords1 = dbf.recordCount - dbf._recordsRead;
            const maxRecords2 = maxCount - records.length;
            ...
            for (let i = 0, offset = 0; i < recordCountToRead; ++i) {
                const record: Record<string, unknown> & {[DELETED]?: true} = {};
                const isDeleted = (buffer.at(offset++) === 0x2a);
                ...
                for (let j = 0; j < dbf.fields.length; ++j) {
                    const field = dbf.fields[j];
                    let len = field.size;
                    let value: any = null;
                    const encoding = getEncodingForField(field, dbf._encoding);

                    // Decode the field from the buffer, according to its type.
                    switch (field.type) {
                        case 'C': // Text
                            while (len > 0 && buffer.at(offset + len - 1) === 0x20) --len;
                        ...
                        case 'F': // Float - appears to be treated identically to Number
                            while (len > 0 && buffer.at(offset) === 0x20) ++offset, --len;
                        ...
                        case 'L': // Boolean
                        {
                            const c = String.fromCharCode(buffer.at(offset++));
                            value = 'TtYy'.indexOf(c) >= 0 ? true : ('FfNn'.indexOf(c) >= 0 ? false : null);
                            break;
                        }
                        case 'T': // DateTime
                            if (buffer.at(offset) === 0x20) {
                                value = null;
                            }
                            else {
                        ...
                        case 'D': // Date
                            value = buffer.at(offset) === 0x20 ? null : parse8CharDate(substrAt(offset, 8, encoding));
                            offset += 8;
                            break;
                        ...
                        case 'M': // Memo
                        {
                            ...
                            value = '';
                            let mergedBuffer = BufferStream.Empty();
                            ...
                            while (true) {
                                ...
                                if (dbf._version === 0x83) {
                                    ...
                                    let eos = memoBuf.indexOf('\x1A' as any as number);
                                    mergedBuffer = BufferStream.concat([mergedBuffer, memoBuf.slice(0, eos === -1 ? memoBlockSize : eos)]);
                                    if (eos !== -1) {
                                ...
                                // Handle first/next block of dBase IV memo data.
                                    ...
                                    const isFirstBlockOfMemo = memoBuf.readInt32LE(0) === 0x0008FFFF;
                                    ...
                                    mergedBuffer = BufferStream.concat([mergedBuffer, memoBuf.slice(skip, skip + take)]);
                                    len -= take;
                                    if (len === 0) {
                                        value = iconv.decode(mergedBuffer, encoding);
                                        break;
                                ...
                                // Handle first/next block of FoxPro9 memo data.
                                    ...
                                    // Read the chunk of memo data, and break out of the loop when all read.
                                    const take = Math.min(len, memoBlockSize - skip);
                                    mergedBuffer = BufferStream.concat([mergedBuffer, memoBuf.slice(skip, skip + take)]);
                                    len -= take;
                                    if (len === 0) {
                                        value = iconv.decode(mergedBuffer, encoding);
...
async function appendRecordsToDBF(dbf: DBFFile, records: Array<Record<string, unknown>>): Promise<DBFFile> {
    ...
    try {
        ...
        fd = await open(dbf.path, 'r+');
        const recordLength = calculateRecordLengthInBytes(dbf.fields);
        const buffer = BufferStream.alloc(recordLength + 4);
...
function validateFieldDescriptors(fields: FieldDescriptor[], fileVersion: number): void {
    ...
    for (const field of fields) validateFieldDescriptor(field, fileVersion);
}

// Private helper function
function validateRecord(fields: FieldDescriptor[], record: Record<string, unknown>): void {
    for (let i = 0; i < fields.length; ++i) {
        const name = fields[i].name, type = fields[i].type;
