...
export const open = async (path: string, flags: string) => {
    ...
    const isRead = flags.indexOf('r') !== -1;
    const isWrite = flags.indexOf('w') !== -1 || flags.indexOf('a')!== -1 ;
    const isPlus = flags.indexOf('+') !== -1;
    const mustNotExist = flags.indexOf('x') !== -1;
    const mustExist = (isRead && isPlus);
    const options = { read: (isRead || (isWrite && isPlus)), write: (isWrite || (isRead && isPlus))};

    if(mustNotExist && fileExists) {
        throw new Error(`File ${path} already exists`);
    } else if (mustExist && !fileExists) {
        throw new Error(`File ${path} does not exist`);
    }

    if(!fileExists && isWrite) {
        return await Deno.create(path)
    }

    return await Deno.open(path, options);
};

export const close = async (file?: Deno.FsFile) => {
    return await file?.close();
}

export const read = async (file: Deno.FsFile, bufferDest: Uint8Array | BufferStream, offset: number, length: number, position?: number) => {
    const buffer = bufferDest instanceof BufferStream  ?
        (bufferDest as BufferStream).buffer : (bufferDest as Uint8Array);
...
export const stat = async (file: Deno.FsFile | string) => {
...
export const write = async (file: Deno.FsFile, bufferSrc: Uint8Array | BufferStream, offset?: number, length?: number, position: number | null = null) => {
    const buffer = bufferSrc instanceof BufferStream  ?
        (bufferSrc as BufferStream).buffer : (bufferSrc as Uint8Array);

    const buf = new Uint8Array(length || buffer.length);
    buf.set(buffer, offset || 0);

    if(position !== null) {
        await file.seek(0, Deno.SeekMode.Start);
    }

    const result = await file.write(buf)
